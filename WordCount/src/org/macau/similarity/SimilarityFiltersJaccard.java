...
public class SimilarityFiltersJaccard implements SimilarityFilters {
    class Partition {
        ...
        public Partition(int startL, int lengthL, int startR, int lengthR,
                int hamming) {
            this.startL = startL;
    ...
    protected final float simThr;
    protected final float simThr100;
    ...
    public int getIntersectUpperBound(int noGramsCommon, int positionX,
    ...
    private Partition getPartition(int[] tokens, int start, int length, int w,
            int posL, int posR) {
        int p;
        if (tokens[posL] > w) {
            p = posL;
        ...
        if (p >= start && p < start + length && tokens[p] == w) {
            return new Partition(start, p - start, p + 1, start + length - p
                    - 1, 0);
        }
    ...
    private int getSuffixFilter(int[] tokensX, int startX, int lengthX,
            int[] tokensY, int startY, int lengthY, int hammingMax, int depth) {
        ...
        Partition partitionX = getPartition(tokensX, startX, lengthX,
                tokensY[mid], Math.max(mid + startX - startY - offset
                        - lengthDiff * offsetL, startX), Math.min(mid + startX
                        - startY + offset + lengthDiff * offsetR, startX
                        + lengthX - 1));

        int hammingPart = partitionX.hamming;

        int hamming = Math.abs(partitionX.lengthL - partitionY.lengthL)
                + Math.abs(partitionX.lengthR - partitionY.lengthR)
                + hammingPart;

        if (hamming <= hammingMax) {
            int hammingL = getSuffixFilter(
                    tokensX,
                    partitionX.startL,
                    partitionX.lengthL,
                    tokensY,
            ...
            if (hamming <= hammingMax) {
                int hammingR = getSuffixFilter(tokensX, partitionX.startR,
                        partitionX.lengthR, tokensY, partitionY.startR,
                        partitionY.lengthR,
                        hammingMax - hammingL - hammingPart, depth + 1);
    ...
    public boolean passLengthFilter(int lengthX, int lengthY) {
        return getLengthLowerBound(lengthX) <= lengthY
                && lengthY <= 1 / simThr * lengthX;
    }
     ...
     * @return
     */
    public boolean passPositionFilter(int noGramsCommon, int positionX,
            int lengthX, int positionY, int lengthY) {
        return getIntersectUpperBound(noGramsCommon, positionX, positionY,
                lengthX, lengthY) >= getIntersectLowerBound(lengthX, lengthY);
    }

    public float passSimilarityFilter(final int[] tokensX, int startX,
            int lengthX, final int prefixLengthX, final int[] tokensY,
            int startY, int lengthY, final int prefixLengthY,
            final int intersectionSizePrefix) {
        ...
        if (token < tokenProbe) {
            ...
            if (intersectionSizePrefix + lengthProbe - prefixLengthY >= intersectSizeLowerBound) {
                intersectSize = intersectionSizePrefix
                        + SimilarityMetric.getIntersectSize(tokensX, startX
                                + intersectionSizePrefix, lengthX
                                - intersectionSizePrefix, tokensY, startY
     ...
     *         otherwise
     */
    public float passSimilarityFilter(final int[] tokensX,
            final int prefixLengthX, final int[] tokensY,
        ...
        return passSimilarityFilter(tokensX, 0, tokensX.length, prefixLengthX,
                tokensY, 0, tokensY.length, prefixLengthY,
                intersectionSizePrefix);
    }

    public boolean passSuffixFilter(int[] tokensX, int tokensStartX,
            int tokensLengthX, int positionX, int[] tokensY, int tokensStartY,
            int tokensLengthY, int positionY) {
        int hammingMax = tokensLengthX
                + tokensLengthY
                - 2
        ...
        int hamming = getSuffixFilter(tokensX, tokensStartX + positionX + 1,
                tokensLengthX - positionX - 1, tokensY, tokensStartY
                        + positionY + 1, tokensLengthY - positionY - 1,
                hammingMax, 1);
        return hamming <= hammingMax;
    }

    public boolean passSuffixFilter(int[] tokensX, int positionX,
            int[] tokensY, int positionY) {
        ...
        // return hamming <= hammingMax;
        return passSuffixFilter(tokensX, 0, tokensX.length, positionX, tokensY,
                0, tokensY.length, positionY);
    }
}
